...
outFile = open('j.json','w')

def getSerpResults(domain):
    global opener
    return opener.open(f'https://www.google.com/search?q={domain}+revenue+%22zoominfo%22&lum_json=1').read()
...
def dump_links(file_path):
    ...
    for i in search_keywords.index:
#put the coloumn heading here
        ...
        if i%save_progress_at == 0:
            pd.DataFrame(data_entries).to_excel(f"save_SerpProgress_{i}.xlsx")
        if os.path.isfile(f"save_SerpProgress_{i-save_progress_at}.xlsx"):
            os.remove(f"save_SerpProgress_{i-save_progress_at}.xlsx")

    pd.DataFrame(data_entries).to_excel("serp_dump.xlsx")
